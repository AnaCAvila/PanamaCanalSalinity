...
################################################################
##### Average Deep levels before and after neopanamax #####
################################################################
library('NLP')


Deep_pre_neo <- subset(Deep, Deep$date < as.Date("2016-06-01") )

averages <- data.frame(unique(Deep_pre_neo$site))
names(averages)[1] <- "site"

count <- 1
for (station in unique(Deep_pre_neo$site)){
  station <- String(station)
  set <- subset(Deep_pre_neo, site==station)
...
Deep_post_neo <- subset(Deep, Deep$date > as.Date("2016-06-01") )

post_neo_averages <- data.frame(unique(Deep_post_neo$site))
names(post_neo_averages)[1] <- "site"

count <- 1
for (station in unique(Deep_post_neo$site)){
  station <- String(station)
  set <- subset(Deep_post_neo, site==station)
...
Deep_pre_neo_same_stations <- data.frame(unique(Deep_post_neo$site))

count <- 1
for (station in unique(Deep_post_neo$site)){
  station <- String(station)
  set <- subset(Deep_pre_neo, site==station)
  Deep_pre_neo_same_stations$average[count] <- mean(set$data)
  count <- count + 1
}
Deep_pre_neo_same_stations

Deep_pre_neo_same_stations <- Deep_pre_neo_same_stations[!is.na(Deep_pre_neo_same_stations$average), ]

mean(Deep_pre_neo_same_stations$average)
...
for (station in unique(distances$site)){
  station <- String(station)
  set <- subset(Deep_pre_neo, site==station)
  set2 <- subset(Deep_post_neo, site==station)
...
plot(lake_distances$min_dist, lake_distances$average, col='lightblue', main="Pre-neopanamax",
     xlab="min distance", ylab="Deep", pch=19)
...
lake_Deep <- Deep[Deep$site %in% (lake), ]


lake_Deep_post_neo <- subset(lake_Deep, lake_Deep$date > as.Date("2016-06-26") )
lake_Deep_post_neo$delta <- ave(lake_Deep_post_neo$data, lake_Deep_post_neo$site, FUN=function(x) c(0, diff(x)))

delay <- data.frame(unique(lake_Deep_post_neo$site))
names(delay)[1] <- "site"

count <- 1
for (station in unique(lake_Deep_post_neo$site)){
  station <- String(station)
  lake_post_neo_station <- subset(lake_Deep_post_neo, lake_Deep_post_neo$site == station)
